...
const links: FooterGroup[] = [
  ...
  {
    group: "Jam Operasional",
    items: [
      {
        title: "Senin—Jumat",
        times: "09.00—18.00 WIB",
        href: "#",
      },
      {
        title: "Sabtu",
        times: "09.00—15.00 WIB",
        href: "#",
      },
      {
        title: "Minggu",
  ...
  {
    group: "For Partnerships/Corporates",
    items: [
      {
...
export default function FooterSection() {
  return (
    <footer className="border-b bg-neutral-900  pt-20 dark">
      <div className="mx-auto sm:max-w-200 lg:max-w-280 xl:max-w-340 px-6 dark:bg-neutral-900">
        <div className="grid gap-12 lg:gap-0 md:grid-cols-6">
          <div className="md:col-span-4 lg:col-span-2">
            <Link href="/" aria-label="go home" className="block size-fit">
              <div className="drop-shadow-xs drop-shadow-slate-50/85">
                <Logo />
              </div>
            </Link>
          </div>

          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 md:col-span-6 lg:col-span-4">
            {links.map((link, index) => (
              <div key={index} className="space-y-4 text-sm">
                <span className="block font-medium text-slate-100 text-md">{link.group}</span>
        ...
        <div className="mt-12 flex flex-wrap items-end justify-between gap-6 border-t py-6">
          <span className="text-muted-foreground order-last block text-center text-sm md:order-first">© {new Date().getFullYear()} Ann Travel Doc, All rights reserved</span>
