...
export const Card = React.memo(({ card, index, hovered, setHovered }: { card: any; index: number; hovered: number | null; setHovered: React.Dispatch<React.SetStateAction<number | null>> }) => (
  ...
  >
    ...
    <div className={cn("absolute inset-0 bg-black/50 flex items-end justify-center gap-7 py-8 px-4 transition-opacity duration-300", hovered === index ? "opacity-100" : "opacity-0")}>
      ...
      <Button asChild size="sm" variant="outline">
        {hovered === index ? (
          <Link href={card.href}>
            <span className="transition-all">Click here for details</span>
          </Link>
        ) : (
          ""
        )}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-5xl mx-auto md:px-8 w-full">
      {cards.map((card, index) => (
        <Card key={card.title} card={card} index={index} hovered={hovered} setHovered={setHovered}></Card>
      ))}
    </div>
  );
}
